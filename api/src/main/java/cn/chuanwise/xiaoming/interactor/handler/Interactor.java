...
public class Interactor {
    String name;
    String[] formats = Arrays.emptyArray(String.class);
    Permission[] permissions = Arrays.emptyArray(Permission.class);
    String usage = null;
    String[] requireGroupTags = Arrays.emptyArray(String.class);
    String[] requireAccountTags = Arrays.emptyArray(String.class);
    ...
    public Interactor(Method method, Plugin plugin) {
        ...
        // 编译参数解析器
        filterMatchers = Arrays.copyAs(filters, FilterMatcher.class, FilterMatcher::filterMatcher);

        // 获得相关权限
        final Required[] permissionAnnotations = method.getAnnotationsByType(Required.class);
        if (permissionAnnotations.length != 0) {
            permissions = Arrays.copyAs(permissionAnnotations, Permission.class, x -> Permission.compile(x.value()));
        }

        // 获得指令用法
        final Usage[] usages = method.getAnnotationsByType(Usage.class);
        final ParameterFilterMatcher firstParameterFilterMatcher = (ParameterFilterMatcher) CollectionUtil.first(java.util.Arrays.asList(filterMatchers), matcher -> (matcher instanceof ParameterFilterMatcher));
        if (usages.length != 0) {
        ...
        requireAccountTags = Arrays.copyAs(method.getAnnotationsByType(RequireAccountTag.class), String.class, RequireAccountTag::value);
        requireGroupTags = Arrays.copyAs(method.getAnnotationsByType(RequireGroupTag.class), String.class, RequireGroupTag::value);
    }
    ...
    public void setFormats(String[] formats) {
        ...
        this.filterMatchers = Arrays.copyAs(formats, FilterMatcher.class, FilterMatcher::parameter);
    }
        ...
        try {
                ...
                if (XiaoMingUser.class.isAssignableFrom(type)) {
                    ...
                    if (Objects.isNull(container)) {
                        ...
                        if (noDot) {
                            ...
                            object = xiaoMingBot.getLanguageManager().calculate(baseVariable, parameterName.substring(dotPosition + 1));
                        }
                    ...
                    // 检查匹配结果
                    if (Objects.isNull(container) || !container.isSet()) {
            ...
            try {
                ...
                if (throwable instanceof InteractExitedException) {
                    ...
                    user.sendError("你已经「" + Times.toTimeLength(timeout) + "」没有回复小明啦，小明就不等待你的下一条消息啦");
