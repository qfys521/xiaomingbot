...
import cn.chuanwise.xiaoming.util.Languages;
...
//        }
        ...
        interactorManager.registerInteractors(new GroupInformationInteractors(), null);
    ...
    private void initialize() {
        ...
        load();
        Languages.config(languageManager);

        consoleXiaoMingUser = new ConsoleXiaoMingUserImpl(new ConsoleContactImpl(this) );
     ...
     * 自定义序列化器，用来存储文件等
     */
    Serializer serializer = Serializers.newJacksonSerializer();

    {
        serializer.getCollectionClassLoader().addClassLoader(xiaoMingClassLoader);
    }

    /**
     * 核心序列化器，用来存储关键文件。例如 configurations 等不能变更序列化器的文件
     */
    final Serializer coreSerializer = Serializers.newJacksonSerializer();

    {
        coreSerializer.getCollectionClassLoader().addClassLoader(xiaoMingClassLoader);
    }

    /**
     * 核心文件载入器
     */
    FileLoader coreFileLoader = new FileLoader(coreSerializer) {
        @Override
        ...
        @Override
        public <T extends Preservable> T loadOrSupply(Class<T> clazz, File file, Supplier<T> supplier) {
            final boolean delayWriteDisabled = Objects.isNull(configuration) || !configuration.isDelayWrite();
            final boolean shouldSave = delayWriteDisabled && !file.isFile();
            final T t = super.loadOrSupply(clazz, file, supplier);
            if (shouldSave && Objects.nonNull(fileSaver)) {
                fileSaver.readyToSave(t);
            }
            return t;
        }
    };
     ...
     * 文件存储信息载入和读取器
     */
    FileLoader fileLoader = new FileLoader(serializer) {
        @Override
        public <T extends Preservable> T load(Serializer serializer, Class<T> clazz, File file) throws IOException {
            final T result = super.load(serializer, clazz, file);
            if (result instanceof XiaoMingObject) {
    ...
    public synchronized void stop() {
        ...
        final XiaoMingStopEvent event = new XiaoMingStopEvent();
